...
| 编号 | 主题 | 涉及模块 | 影响度 | 实施成本 |
| ---- | ---- | -------- | ------ | -------- |
| 1 | `Bidirectional` splice 快路径回归保护 | `base/io/net` | 中 | 小 |
...
建议实施顺序：2 → 6 → 3 → 4 → 5 → 1 → 7 → 11 → 10 → 9 → 8 → 12。先落地收益面广的 TLS "会话恢复"，再推进 Yamux / QUIC 等窗口对齐类改动；§1 本轮定位为"防回归"而非"新开启零拷贝"，优先级下调到中影响度区；最后处理 opt-in 较多、跨平台差异较大的探索性条目。

---

## 1. `Bidirectional` splice 快路径的回归保护

### 现状（Current）

`Bidirectional` 目前通过 `io.CopyBuffer(dst, src, buf)` 已经保留了 `ReaderFrom` / `WriterTo` 的快路径触发能力。Go 标准库 `io/io.go` 中 `copyBuffer` 的文档注释原文为："If either src implements WriterTo or dst implements ReaderFrom, buf will not be used to perform the copy."，也就是即便显式传入 `buf`，只要 `dst` 实现 `io.ReaderFrom` 或 `src` 实现 `io.WriterTo`，标准库依旧会优先调用快路径。`*net.TCPConn` 已实现两者，Linux 平台上实际由 `TCPConn.readFrom` 内部选择 `splice(2)` 或 `sendfile` 完成零拷贝。换言之，TCP 明文转发路径当前已经在走 splice。

一旦未来有改动把 `dst` 包进匿名类型（例如在 `pipe` 内再包一层 stats wrapper）并丢掉 `ReaderFrom`，这条快路径会在无声无息中失效，只会在线上吞吐 / CPU 监控上以"曲线缓慢抬高"出现，难以归因。

### 改动建议（Proposed Change）

把本项的目标从"启用 splice"改为"加入回归保护"，以确保未来修改不会静默退化：

- 在 `base/io/net/copy.go` 或新增的 `base/io/net/copy_test.go` 中加入一段编译期静态断言，放在独立的 `// ensure splice fast path` 注释附近，例如 `var _ io.ReaderFrom = (*net.TCPConn)(nil)`、`var _ io.WriterTo = (*net.TCPConn)(nil)`。这部分仅校验标准库类型关系，不涉及业务逻辑。
- 在 `copy_test.go` 中新增运行时测试：构造一个包裹 `*net.TCPConn` 并计数其 `ReadFrom` 调用的 wrapper，作为 dst 传入 `io.CopyBuffer(wrapped, src, buf)`，断言 wrapper 的 `ReadFrom` 被命中，从而证明 `io.CopyBuffer` 真的走了 `ReaderFrom` 快路径而不是使用 `buf`。
- 另一个配套测试：以 VLESS / Reality / TLS 等典型 wrapped conn 为 dst（模拟其没有 `ReaderFrom` 的情况），断言 `Bidirectional` 仍能正确走 buffered 路径、字节数与错误语义与当前实现一致。
- 可选项（次要收益、不作为核心卖点）：把 `base/io/net/copy.go` 里的 `pipe` 函数改为先用 `io.Copy`，只有在 dst 没有 `ReaderFrom` 时才 fallback 到 buffered 路径，进一步减少 `sync.Pool` 的 `Get` / `Put` 调用。

### 预期收益（Expected Gain）

- 防止未来提交无意中破坏 splice 快路径，把"是否走零拷贝"这个关键假设从口头约定升级为 CI 可验证的断言。
- 可选改动若落地，额外顺带减少 `sync.Pool` 的 `Get` / `Put` 次数，但这是附属收益。

### 风险（Risks）

- 测试需覆盖 wrapped conn（TLS / VLESS / Reality / PHT `clientConn` 等）以确认 buffered 路径仍工作，且 `closeWrite` / `normalizeCopyError` / `waitSecondResult` 语义在两条路径下保持一致。
- 若采用"优先 `io.Copy`、fallback 到 buffered"的可选改动，需要跑 race test 覆盖半关闭边界，避免 `io.Copy` 与 `CopyBuffer` 在错误返回时机上出现语义差异。

### 回退开关（Opt-out）

无需开关；当前行为就是保留 splice 快路径，本项只是加防回归断言。如果未来采纳可选改动后出现意外问题，使用环境变量 `FORWARD_COPY_FORCE_BUFFERED=1` 做紧急回退，强制走 buffered 路径。默认不设置。
...
Reality（`internal/dialer/reality/dialer.go`）走 xray 内部握手，不在本项范围内。`internal/dialer/hysteria2/dialer.go` 的 `buildClientConfig` 走 hy2 自带 TLS 栈（`hyclient.Config` 内部 TLS），不经过 `internal/config/tls.ClientConfig`，本优化项不覆盖 hy2 路径（如需 hy2 session resumption 由上游 `hyclient` 库自行处理）。
...
- 环境变量 `FORWARD_TLS_CACHE_DISABLE=1` 全局关闭。
默认开启。
...
3. 对 `internal/chain/route_impl.go`、`internal/chain/balancer.go`、`internal/xraymux/route_dispatcher.go`、`internal/handler/tcp/handler.go`、`internal/handler/udp/handler.go` 中的热点 `Debug(...)` 调用加 `if tr.Verbose && tr.Logger.Level() <= logging.LevelDebug` 守卫，避免在非 debug 级别下构造 args（例如多次调用 `RouteSummary`、`labelNode`）。`Logger` 需要暴露 `Level()` 这一公开方法（已存在）。
...
单节点 UDP 转发 PPS 预期显著提升，显著降低 syscall 次数与 CPU 占用。对游戏、视频流、WireGuard 中继等高 PPS 场景收益最大。
...
本 Spec 引入或涉及的环境变量统一遵循 `FORWARD_<MODULE>_<ACTION>` 格式，其中 `<MODULE>` 指向受影响的子模块（TLS / COPY / BALANCER 等），`<ACTION>` 描述动作或目标（DISABLE / FORCE_BUFFERED / PROBE_URL 等）。当前 Spec 范围内涉及：

- `FORWARD_TLS_CACHE_DISABLE`：全局关闭 TLS ClientSessionCache（见 §2）。
- `FORWARD_COPY_FORCE_BUFFERED`：紧急回退开关，强制 `Bidirectional` 走 buffered 路径（见 §1 可选改动）。
- `FORWARD_BALANCER_PROBE_URL`：Balancer 可选 CONNECT+204 测速的全局默认 URL（见 §8）。

后续新增环境变量（无论是本 Spec 的后续迭代还是其他优化项）都应保持同一命名结构，避免 `DISABLE_X` 与 `X_DISABLE` 混用。
