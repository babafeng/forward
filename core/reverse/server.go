...
func handleConnection(conn net.Conn, scheme string, auth *utils.Auth) {
	...
	utils.Logging("Peeked byte: 0x%02x scheme: %s", peek[0], scheme)
...
func handleBind(conn net.Conn, header []byte) {
	...
	utils.Info("Client connected: %s, Session ID: %s, Bound: :%d", conn.RemoteAddr(), sessionID, bindPort)
	...
	go func() {
		...
		for {
			...
			go func() {
				...
				defer stream.Close()
				utils.Transfer(userConn, stream, "tunnel", "ReverseServer", "TCP")
			}()
		}
	}()
}
